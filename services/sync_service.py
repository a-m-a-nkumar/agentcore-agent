...
async def sync_project(project_id: str, user_id: str, sync_type: str = 'incremental'):
    ...
    try:
        ...
        _update_sync_status(project_id, is_syncing=False, message="Sync completed — vector DB is up to date.")
...
async def sync_confluence_space(
    ...
    _update_sync_status(project_id, message=f"Fetching page list from Confluence space '{space_key}'...")
    ...
    _update_sync_status(project_id, message=f"Found {len(pages)} Confluence pages — checking for changes...")
    ...
    for idx, page in enumerate(pages):
        try:
            ...
            _update_sync_status(project_id, message=f"Confluence [{idx+1}/{len(pages)}]: Fetching '{page_title}' from API to check version...")
            ...
            response = await asyncio.to_thread(
                requests.get, page_url, headers=confluence.headers, auth=confluence.auth, timeout=15
            )
            ...
            needs_update = False
            if not local_page:
                # New page — not in our metadata DB yet
                logger.info(f"  New page: {full_page['title']}")
                _update_sync_status(project_id, message=f"Confluence [{idx+1}/{len(pages)}]: '{full_page['title']}' is NEW — syncing & embedding...")
                needs_update = True
            elif local_page['version_number'] < version_number:
                # Updated page — Confluence version is newer than our stored version
                logger.info(f"  Updated page: {full_page['title']} (v{local_page['version_number']} → v{version_number})")
                _update_sync_status(project_id, message=f"Confluence [{idx+1}/{len(pages)}]: '{full_page['title']}' changed (v{local_page['version_number']} → v{version_number}) — re-embedding...")
                needs_update = True
            else:
                # Unchanged page — version matches, skip
                logger.debug(f"  Skipping unchanged page: {full_page['title']}")
                _update_sync_status(project_id, message=f"Confluence [{idx+1}/{len(pages)}]: '{full_page['title']}' unchanged (v{version_number}) — skipped")
                skipped_count += 1
                continue
    ...
    _update_sync_status(project_id, message=f"Confluence done — {synced_count} updated, {skipped_count} unchanged. Moving to Jira...")
...
async def sync_jira_project(
    ...
    _update_sync_status(project_id, message=f"Fetching issue list from Jira project '{project_key}'...")
    ...
    _update_sync_status(project_id, message=f"Found {len(issues)} Jira issues — checking for changes...")
    ...
    for idx, issue in enumerate(issues):
        try:
            ...
            _update_sync_status(project_id, message=f"Jira [{idx+1}/{len(issues)}]: Checking '{issue_key}' — comparing updated timestamps...")
            ...
            needs_update = False
            if not local_issue:
                # New issue — not in our metadata DB yet
                logger.info(f"  New issue: {issue_key}")
                _update_sync_status(project_id, message=f"Jira [{idx+1}/{len(issues)}]: '{issue_key}' is NEW — syncing & embedding...")
                needs_update = True
            else:
                ...
                if local_updated and updated_date:
                    ...
                    if local_comp < updated_comp:
                        ...
                        _update_sync_status(project_id, message=f"Jira [{idx+1}/{len(issues)}]: '{issue_key}' updated ({local_comp} → {updated_comp}) — re-embedding...")
                        needs_update = True
                else:
                    needs_update = True

            if not needs_update:
                # Unchanged issue — timestamps match, skip
                logger.debug(f"  Skipping unchanged issue: {issue_key}")
                _update_sync_status(project_id, message=f"Jira [{idx+1}/{len(issues)}]: '{issue_key}' unchanged — skipped")
                skipped_count += 1
                continue
            ...
            if needs_update:
                ...
                # Generate new embeddings
                content = f"{fields['summary']}\n\n{_strip_html(fields.get('description', ''))}"
                await generate_and_store_embeddings(
                    project_id=project_id,
                    user_id=user_id,
                    source_type='jira',
    ...
    _update_sync_status(project_id, message=f"Jira done — {synced_count} updated, {skipped_count} unchanged.")
